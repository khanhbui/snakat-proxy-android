...
namespace snakat {
    ...
    bool Proxy::init(AAssetManager *assetManager, const std::string &documentsAbsDir,
                     const std::string &hostname, bool sslEnabled,
                     const std::string &sslCertFilename, const std::string &manifestFilename) {
        LOGI("Proxy's config: {\n\tdocumentsAbsDir: %s, \n\thostname: %s, \n\tsslEnabled: %d, \n\tsslFilename: %s, \n\tmanifestFilename: %s\n}",
        ...
        _fileHandler = FileHandler::create(documentsAbsDir, manifestFilename);
